...
* **데이터 기간 및 크기:** 2021-09-10 ~ 2026-09-09 (최근 5년), 총 1,257개 데이터 포인트.
...
| :--- | :--- | :--- | :--- | :--- |
| 2021 | 0.45 | 2.15 | -5.20 | 6.80 |
| (터미널에 출력된 실제 표 내용을 여기에 복사해 넣습니다) |

---
**(인사이트 섹션에 추가할 내용)**

**💡 분석 질문 4 (추가): 월별로 노이즈(이상치)가 집중되는 계절적 패턴이 존재하는가?**
...
## 5. 결론 및 한계점
* **결론:** 시계열 데이터 분석 기법을 통해, 매일 요동치는 가격 변화(노이즈)에 속지 않고 60일 이동평균선으로 시장의 '진짜 방향'을 읽어낼 수 있었다. 또한 코드를 통한 변동성과 이격도 추출은 감이나 뉴스에 의존하지 않는 객관적인 리스크 관리 지표가 됨을 확인했다.
...
## 6. AI 사용 로그
* **사용 작업:** 프로젝트 초기 환경 구축 가이드, `yfinance` 튜플 반환 오류 해결(호환성 패치), 분석 파이프라인 구조화 및 상세 주석 작성.
...
* **검증 방법:** 제공된 코드를 로컬 가상환경에서 직접 실행하며 `print` 문으로 출력된 팩트 수치들을 검증했고, 해당 수치가 리포트의 결론과 논리적으로 정확히 부합하는지 교차 검증함.